...
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------- | -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BootScene.ts`                         | Splash + asset preload, hands off to `GameScene`. Hosts the title screen layout: title (top), tagline, animated stone-arched door (`boot_door` 2-frame spritesheet, frame 0 closed / 1 open), torches flanking the door, "Start" button below the door. Clicking the button plays the procedural `doorOpen` SFX, swaps the door to frame 1, then fades to `GameScene`. **Audio:** the title screen is musically silent — `music.stop()` is called on entry (kills any music bled in from a restart), and a procedural `sfx.startTorchAmbient(...)` is fired at the same beat as the torches catching. The Start handler calls `sfx.stopTorchAmbient(...)` before the camera fade so GameScene starts in silence; `GameScene.startGame` is where the dungeon playlist is set + `music.start()` happens. | —                                 | `Localization`, `SoundManager`, `MusicManager`                                                     | The **shared** `Localization`, `SoundManager`, `MusicManager` instances passed to all later scenes. Restarts preserve language and audio state.                                          |
...
| `SoundManager.ts`                                                        | SFX bank + ambient loops, persisted mute flag. Mostly procedural Web-Audio synth cues, but the UI hover / click feedback are sampled from `public/audio/ui_hover.ogg` and `ui_click.ogg`. **`preloadUiSfx()` is called from `BootScene.create()` to fetch + decode them at startup; if the buffer is missing the cue falls back to the old synth (click) or stays silent (hover).** `sfx.play('roomHover')` is wired to map-node `pointerover` in `MapView` for reachable nodes only — don't fire it on every node. Two distinct ambient loops: **`startAmbient(depth)` / `stopAmbient()`** is the deep dungeon drone used in `GameScene`; **`startTorchAmbient(fadeMs)` / `stopTorchAmbient(fadeMs)`** is the boot-screen-only filtered-noise crackle (hiss + brown-noise rumble + randomly scheduled high-passed "pops"). They can coexist but in practice `BootScene` owns the torch loop and `GameScene` owns the dungeon drone. | —                                                                                                                                         | `Phaser.Sound`, `localStorage`, `fetch` + `AudioContext.decodeAudioData` for sampled UI SFX                                | SFX cache (procedural + decoded sample buffers), dungeon-drone ambient nodes, torch-crackle ambient layers + pop timer.                  |
...
| `HudCell.ts` / `HudFrame.ts` / `HudIcons.ts` / `HudTheme.ts` | Bottom-bar resource cells, top/bottom carved frame, icon spritesheet bindings, palette. **`HudTheme.ts` also exports the two centralised font stacks `HUD_FONT` (JetBrains Mono — HUD / log / numbers) and `BODY_FONT` (EB Garamond — room copy / action buttons / end-screens). Files live in `public/fonts/` and are registered via `@font-face` in `src/style.css`; `main.ts` blocks Phaser boot on `document.fonts.ready` so the canvas Text objects never start on a system fallback. Do NOT inline a fontFamily literal in new code — import the constant.** | HUD widget factories.                                                     |
| `MapView.ts`                                                 | Map node visuals + tweens + pointer interaction.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Node sprites, edge graphics, hover/pulse tweens.                          |
| `RelicSlots.ts`                                              | Inline 5-cell relic display in the bottom HUD bar with hover tooltips (name + description + rarity badge). Each cell is a `panel_small.png` nine-slice (via `drawPanel`) with a rarity-coloured stroke overlay; empty slots paint the panel at low alpha to read as available capacity. Owns its own `relicsChange` subscription; built once in `GameHudController.buildRelicSlots`.                                                                                                                                                                               | Slot/tooltip widget refs.                                                 |
