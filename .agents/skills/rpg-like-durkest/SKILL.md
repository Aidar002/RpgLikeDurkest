...
| File                       | Role                                                                                                                                                                                           |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GameHudController.ts`     | Owns the global HUD: top + bottom bars, escape/restart buttons, restart-confirm modal, torchlight overlay, refresh + player-event wiring, relic slot row. **~900 lines** — biggest controller. |
...
| File                                                                     | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CombatManager.ts`                                                       | Turn combat: enemy intents, status effects, rewards, boss phase machine. Takes optional seeded `Rng`. Emits `enemyUpdate`, `playerStatusChange`, `enemyStatusChange`, `playerHit`, `combatEnd`. **~1 070 lines** — second largest.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `EnemyPicker.ts`                                                         | `getEnemyForDepth(depth, rng)` — picks a normal-tier enemy from `ENEMY_TIERS`. Lives here (not in `data/`) because it consumes an `Rng` and the `data → systems → ui → scenes` rule forbids value-imports out of `systems/` from `data/`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `MapGenerator.ts`                                                        | Procedural room graph: layer build, boss placement, seal coverage, weighted room rolls. Takes optional seeded `Rng`. Re-exports `MapTypes` for back-compat.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `map/seals.ts`                                                           | `getRequiredSeals(runLength)` + `POST_MAJOR_RECOVERY_POOL`. Pure constants + a tiny helper.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `map/validate.ts`                                                        | `validateMap` (post-build invariant report) + `formatMapDebug` + the four per-path scoring helpers used by `MapGenerator`. All pure.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `NpcManager.ts` + `Npcs.ts`                                              | NPC altar offer rolling and post-pick state. ⚠️ `pickForRole` / `pickBossIntro` still call `Math.random()` directly.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `Skills.ts`                                                              | Skill catalog + `STARTER_LOADOUT`. ⚠️ `SkillDef.starter: boolean` is declared but never read (dead field).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `SoundManager.ts`                                                        | SFX bank + ambient loops + persisted mute. Mostly procedural Web-Audio synth; UI + torch SFX are sampled (`public/audio/ui_hover.ogg`, `ui_click.ogg`, `torch_ignite.mp3`, `torch_loop.mp3`, preloaded in `BootScene.create()` via `preloadUiSfx`). `buttonHover` / `roomHover` both play `ui_hover.ogg`; `buttonClick` / `nodeSelect` both play `ui_click.ogg`; `torchIgnite` plays `torch_ignite.mp3` (procedural flint+whoosh+sub fallback). Two ambient loops: `startAmbient(depth)` (dungeon drone) vs `startTorchAmbient(fadeMs)` — when `torch_loop.mp3` is preloaded the latter layers two loop sources at different start offsets + playback rates to desync the two flanking torches and the ~10 fps boot-torch animation, otherwise falls back to the procedural noise+pops crackle. **~1 110 lines** — biggest file. |
...
| File                 | What's inside                                                                                                                                                                                                                    |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GameConfig.ts`      | `PLAYER_CONFIG`, `LEVEL_UP_CONFIG`, `EXPEDITION_CONFIG`, `COMBAT_CONFIG`, `MAP_CONFIG`, `ROOM_CONFIG`, `ENEMY_TIERS`, `BOSSES`, `ALTAR_EFFECTS`, `XP_CONFIG`, `FEATURES` (feature flags).                                        |
| `MapTypes.ts`        | `RoomType` (const-object enum), `BossKind`, `SealType`, `MapNode`. Re-exported by `MapGenerator` for back-compat.                                                                                                                |
| `Enemies.ts`         | `getBossForDepth(depth)`, `EXPECTED_BOSS_NAMES`, `assertBossMapping()` (validates `BOSSES` at module load). Non-boss enemy pools live in `GameConfig.ENEMY_TIERS`; pick them via `getEnemyForDepth` in `systems/EnemyPicker.ts`. |
