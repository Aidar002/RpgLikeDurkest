...
| `SoundManager.ts`                                                        | SFX bank + ambient loops + persisted mute. Mostly procedural Web-Audio synth; UI + torch + door SFX are sampled (`public/audio/ui_hover.ogg`, `ui_click.ogg`, `torch_ignite.mp3`, `torch_loop.mp3`, `door_in_dungeon2.mp3`, preloaded in `BootScene.create()` via `preloadUiSfx`). `buttonHover` / `roomHover` both play `ui_hover.ogg` (gain 1.6); `buttonClick` / `nodeSelect` both play `ui_click.ogg` (gain 2.2 — both boosted ~2× over the historical 0.8/1.1 so UI feedback cuts through the dungeon ambience, with the synth fallbacks volume-bumped to match); `torchIgnite` plays `torch_ignite.mp3` (procedural flint+whoosh+sub fallback); `doorOpen` **layers** `door_in_dungeon2.mp3` on top of the procedural creak+thud — the sample doesn't short-circuit the synth, so a missing buffer just falls through to synth-only. Two ambient loops: `startAmbient(depth)` (dungeon drone) vs `startTorchAmbient(fadeMs)` — when `torch_loop.mp3` is preloaded the latter layers two loop sources at different start offsets + playback rates to desync the two flanking torches and the ~10 fps boot-torch animation, otherwise falls back to the procedural noise+pops crackle. **~1 110 lines** — biggest file. |
