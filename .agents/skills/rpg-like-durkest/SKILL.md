...
| `SoundManager.ts`                                                        | SFX bank + ambient loops + persisted mute. Mostly procedural Web-Audio synth; UI hover/click are sampled (`public/audio/ui_hover.ogg` / `ui_click.ogg`, preloaded in `BootScene.create()`). Two ambient loops: `startAmbient(depth)` (dungeon drone) vs `startTorchAmbient(fadeMs)` (boot-screen torch crackle). **~1 110 lines** — biggest file. |
...
| `UiButton.ts`                                                | `drawUiButton` — six `btn_*` carved-stone variants as nine-slice + centered label, with procedural fallback when texture missing.                                                                                                                                                                                    |
