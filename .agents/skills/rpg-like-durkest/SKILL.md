...
| File                                                                     | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CombatManager.ts`                                                       | Turn combat: enemy intents, status effects, rewards, boss phase machine. Takes optional seeded `Rng`. Emits `enemyUpdate`, `playerStatusChange`, `enemyStatusChange`, `playerHit`, `combatEnd`. **~1 020 lines** — orchestration only; player-side handlers, relic on-attack hooks, Mime/Mammon helpers, and the boss-turn runner live in `combat/` sub-modules. The file header carries a `[FIX-N]` dictionary explaining every `[FIX-1] / [FIX-5] / [FIX-10] / [FIX-13]` tag scattered through the body.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `combat/BossTurn.ts`                                                     | Boss-turn runner (`runBossTurn`) and internal `resolveBossWindupAction`. Owns phase advancement, multi-turn windup tick/resolution, action dispatch (hero-call drain, mime random-status, lifesteal, heal-on-safe, true-damage), and boss-block tick. Pure function taking a `BossTurnDeps` bundle (which carries a `MimeChaosDeps` for the Mime / Mammon sub-calls). ~330 lines.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
4. **Combat wiring** — `src/systems/combat/BossTurn.ts`
   (orchestration entry point: `src/systems/CombatManager.runBossTurn`)
   - Usually nothing. The `runBossTurn` / `resolveBossWindupAction`
     machinery in `BossTurn.ts` (plus `resolvePrepare` in
     `BossRuntime.ts`) reads the blueprint generically. Edit only
     for a brand-new action kind.
...
- `GameScene` is now a thin coordinator (~460 lines) that delegates
  ...
  `src/ui/EffectsLibrary.ts` (~1 580 lines: VFX recipe registry).
  `src/systems/CombatManager.ts` is ~1 020 lines (orchestration +
  state-machine; `BossRuntime`, `EnemyTurn`, `combat/PlayerActions`,
  `combat/RelicHooks`, `combat/MimeChaos`, and `combat/BossTurn`
  are factored out next to it). `src/ui/end/DeathScreen.ts`
  (~1 150 lines:
