...
| `HudCell.ts` / `HudFrame.ts` / `HudIcons.ts` / `HudTheme.ts` | Bottom-bar resource cells, top/bottom carved frame, icon spritesheet bindings, palette. **`HudTheme.ts` also exports the two centralised font stacks `HUD_FONT` (HUD / log / numbers) and `BODY_FONT` (room copy / action buttons / end-screens). Do NOT inline `'Lucida Console …'` or `'Courier New'` in new code — import the constant.**                                         | HUD widget factories.                                                     |
