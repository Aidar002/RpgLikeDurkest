...
describe('MapGenerator', () => {
    ...
    it('emits no BOSS rooms before the final layer', () => {
        // PR-1 removes hardcoded boss depths. The only BOSS rooms
        // generated by this PR live on the final layer
        // (`depth === runLength`). Mid-run major / mini bosses
        // are placed by the bossPressure pass in PR-2.
        const runLength = 25;
        const { nodes } = generateFullRun(runLength, 123);
        const earlyBosses = nodes.filter(
            (n) => n.type === RoomType.BOSS && n.depth < runLength,
        );
        expect(earlyBosses).toHaveLength(0);
    });
    ...
    describe('runLength scaling (PR-1 architecture)', () => {
        ...
        it('passes validateMap on every seed for the canonical runLength', () => {
            for (let seed = 0; seed < 30; seed++) {
                ...
                // PR-1 only generates final bosses, so there must
                // never be a boss-to-boss edge.
