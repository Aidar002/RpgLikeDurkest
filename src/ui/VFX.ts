...
export class VFX {
    ...
    static critFlash(scene: Phaser.Scene) {
        const flash = scene.add
            .rectangle(CENTER_X, CENTER_Y, GAME_WIDTH, GAME_HEIGHT, 0xffc800, 0.22)
            .setDepth(89);
        scene.tweens.add({
            targets: flash,
            alpha: 0,
    ...
    static shieldFlash(scene: Phaser.Scene, x: number, y: number) {
        const shield = scene.add
            .rectangle(x, y, 60, 60, 0x4488ff, 0.3)
            .setDepth(87)
    ...
    static nodeFire(
        ...
        // between cycles.
        const haloOutermost = scene.add.circle(x, y + 4, 38, 0xff7a22, 0.1).setDepth(7);
        const haloMid = scene.add.circle(x, y + 4, 24, 0xffa040, 0.2).setDepth(8);
        ...
        const spawnEmber = () => {
            ...
            const dot = scene.add.circle(startX, startY, radius, col, 0.86).setDepth(12);
            parent.add(dot);
            particles.add(dot);
        ...
        const spawnFlicker = () => {
            if (!alive) return;
            const flash = scene.add.circle(x, y + 6, 7, 0xfff0c0, 0.78).setDepth(11);
            parent.add(flash);
            particles.add(flash);
            scene.tweens.add({
                targets: flash,
    ...
    static ambientEmbers(scene: Phaser.Scene, count = 18) {
        const spawn = () => {
            ...
            const dot = scene.add
                .rectangle(x, y + 20, sz, sz, col, 0.4 + Math.random() * 0.2)
                .setDepth(3);
            scene.tweens.add({
                targets: dot,
                ...
                onComplete: () => {
                    dot.destroy();
                    scene.time.delayedCall(Math.random() * 600, spawn);
                },
            });
        };
